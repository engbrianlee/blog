...
import ToggleWithContext from "./ToggleWithContext";
...
const useDarkMode = () => {
  ...
  // since there is not way to get local storage or do a media query.
...
Fundamentally, since we use pure Javascript, we have the most flexibility. We can tailor the response of each component to whatever the current theme is. This is a great fit for components that have functional differences between themes as well as for integrating a theming solution into an existing project. If you only really care about changing colors I would recommend the above approach of CSS Variables.

This was the approach taken for this site as it was forked from the [TailwindCSS Blog](https://github.com/tailwindcss/blog) and I didn't want to rewrite the site to use CSS Variables. I was easily able to create a dark theme by adding some dynamic styles to a couple of components.
